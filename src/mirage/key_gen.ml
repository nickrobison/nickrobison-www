...
let build_id = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.string "1698036949." (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "BUILD")
   ?doc:(Some "Build number  ") ?env:(None) ["build-id"]))

let build_id_t = Functoria_runtime.Key.term build_id

let build_id () = Functoria_runtime.Key.get build_id
...
let goodreads = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.string "" (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "KEY")
   ?doc:(Some "API Key for Goodreads page  ")
   ?env:(Some (Cmdliner.Cmd.Env.info "GOODREADS")) ["goodreads"]))

let goodreads_t = Functoria_runtime.Key.term goodreads

let goodreads () = Functoria_runtime.Key.get goodreads

let goodreads_user = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.string "15176504" (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "USER")
   ?doc:(Some "Goodreads user  ") ?env:(None) ["goodreads-user"]))

let goodreads_user_t = Functoria_runtime.Key.term goodreads_user

let goodreads_user () = Functoria_runtime.Key.get goodreads_user

let host = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.string "localhost" (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "URL")
   ?doc:(Some "Hostname of the unikernel  ")
   ?env:(Some (Cmdliner.Cmd.Env.info "HOST")) ["host"]))

let host_t = Functoria_runtime.Key.term host

let host () = Functoria_runtime.Key.get host

let http_port = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.int (80) (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "PORT")
   ?doc:(Some "Port to listen on for plain HTTP connections  ") ?env:
   (None) ["http-port"]))
...
let https_port = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.int (443) (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "PORT")
   ?doc:(Some "Port to listen in for HTTPS connections  ") ?env:(None)
   ["https-port"]))
...
let page_lifetime = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.int (60) (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(Some "TIME")
   ?doc:(Some "Lifetime of the index page cache (in minutes).  ") ?env:
   (None) ["page-lifetime"]))

let page_lifetime_t = Functoria_runtime.Key.term page_lifetime

let page_lifetime () = Functoria_runtime.Key.get page_lifetime
...
let redirect = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt (Cmdliner.Arg.some Cmdliner.Arg.string) 
   (None) (Cmdliner.Arg.info ~docs:"APPLICATION OPTIONS" ?docv:(Some "URL")
   ?doc:(Some
           "Where to send the redirects. Must start with http:// or https://. When tls is enabled, the default is https://\\$HOST.  ")
   ?env:(Some (Cmdliner.Cmd.Env.info "REDIRECT")) ["redirect"]))

let redirect_t = Functoria_runtime.Key.term redirect

let redirect () = Functoria_runtime.Key.get redirect

let resolver = Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt (Cmdliner.Arg.some (Cmdliner.Arg.list Cmdliner.Arg.string)) 
...
let runtime_keys = List.combine [allocation_policy_t; backtrace_t;
                                 build_id_t; custom_major_ratio_t;
                                 custom_minor_max_size_t;
                                 custom_minor_ratio_t; gc_verbosity_t;
                                 gc_window_size_t; goodreads_t;
                                 goodreads_user_t; host_t; http_port_t;
                                 https_port_t; ipv4_t; ipv4_only_t; ipv6_t;
                                 ipv6_only_t; logs_t; major_heap_increment_t;
                                 max_space_overhead_t; minor_heap_size_t;
                                 page_lifetime_t; randomize_hashtables_t;
                                 redirect_t; resolver_t; space_overhead_t] 
["allocation-policy"; "backtrace"; "build-id"; "custom-major-ratio";
 "custom-minor-max-size"; "custom-minor-ratio"; "gc-verbosity";
 "gc-window-size"; "goodreads"; "goodreads-user"; "host"; "http-port";
 "https-port"; "ipv4"; "ipv4-only"; "ipv6"; "ipv6-only"; "logs";
 "major-heap-increment"; "max-space-overhead"; "minor-heap-size";
 "page-lifetime"; "randomize-hashtables"; "redirect"; "resolver";
 "space-overhead"]
